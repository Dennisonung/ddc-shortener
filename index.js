...
function g(h) {
    ...
    existsSync(`./ShortData/${e}.json`) && !j.parse(readFileSync(`./ShortData/${e}.json`, 'utf8')).e == e && (e = sha(k).slice(0, 8));
    writeFileSync(`./ShortData/${e}.json`, j.stringify({u: k}), (err) => { console.log(err) });
    return "http://localhost:8069/" + e;
};
a.get('/', (_req, res) => res.sendFile(__dirname + "/index.html"));
...
a.get('/:e', (req, res) => {
    existsSync(`./ShortData/${req.params.e}.json`) ? res.redirect(j.parse(readFileSync(`./ShortData/${req.params.e}.json`, 'utf8')).u) : res.redirect("/");
});
a.listen(8069);
